    ...
    >
      ...
      <div style={{ padding: 20, paddingTop: 80 }} className="modal-body">
        ...
        <div className="tools mb20">
          {item.tools.map((tool) => (
            <a href={tool.url} target="_blank" key={tool.url}>
              {
                Object.entries($with).find(
                  (entry) => entry[1].url == tool.url
                )?.[0]
              }
            </a>
          ))}
    ...
    <>
      <div className="header">
        ...
        <p>
          ...
          <a href="https://x.com/bandinopla" target="_blank">
            @bandinopla
          </a>
          <a href="https://www.artstation.com/bandinopla" target="_blank">
            ArtStation
          </a>
          <a
            href="https://sketchfab.com/pablobandinopla/models"
            target="_blank"
          >
            Sketchfab
          </a>
          <a href="https://medium.com/@pablobandinopla" target="_blank">
            Articles
          </a>
      ...
      <div className="header">
        <h2 className="mb60">Looking for my next creative challenge!</h2>
      </div>
    </>
  );
}
...
function VideoThumbnail({ id }: { id: string }) {
  ...
  useLayoutEffect(() => {
    ...
    if (isTouchDevice()) {
      ...
      unsubs.push(() =>
        window.removeEventListener("scroll", checkAndToggleVideo)
      );
      unsubs.push(() =>
        window.removeEventListener("resize", checkAndToggleVideo)
      );
    }
    ...
    unsubs.push(() =>
      vref.current!.removeEventListener(
        "mouseenter",
        onMouseEnter as EventListener
      )
    );
    unsubs.push(() =>
      vref.current!.removeEventListener(
